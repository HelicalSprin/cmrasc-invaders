...
export class UIManager {
  constructor(documentRef = document) {
    ...
    this.elements = {
      ...
      upgradeToast: this.byId("upgrade-toast"),
  ...
  updateBuffBar(powerUps) {
    ...
    if (powerUps.reloadBonus > 0) {
      const pill = this.document.createElement("div");
      pill.className = "buff-pill";
      pill.textContent = `${SYMBOLS.lightning} -${powerUps.reloadBonus}`;
      this.elements.buffbar.appendChild(pill);
    }

    if (powerUps.bulletSpeedBonus > 0) {
      const pill = this.document.createElement("div");
      pill.className = "buff-pill";
      pill.textContent = `🔥 SPD+${powerUps.bulletSpeedBonus}`;
      this.elements.buffbar.appendChild(pill);
    }

    if (powerUps.moveSpeedBonus > 0) {
      const pill = this.document.createElement("div");
      pill.className = "buff-pill";
      pill.textContent = `💨 MOV+${powerUps.moveSpeedBonus.toFixed(1)}`;
      this.elements.buffbar.appendChild(pill);
    }
  }

  // ── Non-obtrusive upgrade toast — appears center-screen for 2 s then fades ──
  showUpgradeToast(upgrade) {
    const toast = this.elements.upgradeToast;
    if (!toast) return;

    this.byId("ut-icon").textContent  = upgrade.icon;
    this.byId("ut-name").textContent  = upgrade.name;
    this.byId("ut-desc").textContent  = upgrade.desc;
