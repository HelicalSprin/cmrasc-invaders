...
export class EnemyManager {
  ...
  updateSwarm() {
    ...
    alive.forEach((enemy) => {
      ...
      // REBALANCED: slightly reduced fire chance
      const fireChance = 0.00006 + this.game.state.wave * 0.00006;
      if (Math.random() < fireChance) {
        ...
        if (lowest === enemy) {
          this.game.bulletManager.shootAt(
            ...
            this.game.player.x,
            this.game.player.y,
            // REBALANCED: bullet speed cap reduced
            2.5 + this.game.state.wave * 0.2,
          );
        }
      }
